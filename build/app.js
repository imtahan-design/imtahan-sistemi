...
function hashPassword(p){try{if(typeof bcrypt!=='undefined'&&bcrypt.hashSync)return'bcrypt:'+bcrypt.hashSync(p,10)}catch(e){}try{if(typeof CryptoJS!=='undefined'&&CryptoJS.SHA256)return'sha256:'+CryptoJS.SHA256(p).toString()}catch(e){}return null}
function verifyPassword(p,h){if(!h)return false;if(h.startsWith('bcrypt:')){try{return bcrypt.compareSync(p,h.slice(7))}catch(e){return false}}if(h.startsWith('$2')){try{return bcrypt.compareSync(p,h)}catch(e){return false}}if(h.startsWith('sha256:')){try{return CryptoJS.SHA256(p).toString()===h.slice(8)}catch(e){return false}}return false}
...
async function loadData() {
    ...
    // --- "Dərslik" və "Dövlət qulluğu" bərpası ---
    let dq = categories.find(c => c.name.trim() === 'Dövlət qulluğu');
    let derslik = categories.find(c => c.name.trim() === 'Dərslik');

    // 1. Dövlət qulluğu ID-sini qoru (alt bölmələr itməsin deyə)
    if (!dq) {
        // Əgər DQ yoxdursa, '1' ID-si ilə yaradırıq (alt bölmələr adətən '1' parentId istifadə edir)
    ...
    // 2. Dərslik kateqoriyasını yarat
    if (!derslik) {
    ...
    // 3. Azərbaycan tarixi kateqoriyasını 'derslik_default' altına bağla
    let azHistory = categories.find(c => c.name.trim() === 'Azərbaycan tarixi');
    if (azHistory) {
        if (azHistory.parentId !== 'derslik_default') {
            azHistory.parentId = 'derslik_default';
            if (db) db.collection('categories').doc(String(azHistory.id)).update({ parentId: 'derslik_default' });
            hasChanged = true;
        }
    } else {
        azHistory = { id: 'az_history_cat', name: 'Azərbaycan tarixi', time: 45, questions: [], parentId: 'derslik_default' };
        categories.push(azHistory);
        if (db) db.collection('categories').doc('az_history_cat').set(azHistory);
        hasChanged = true;
    }

    // 4. XI sinif kateqoriyasını 'az_history_cat' altına bağla
    let history11 = categories.find(c => c.name.trim() === 'XI sinif');
    if (history11) {
        if (history11.parentId !== 'az_history_cat') {
            history11.parentId = 'az_history_cat';
            if (db) db.collection('categories').doc(String(history11.id)).update({ parentId: 'az_history_cat' });
            hasChanged = true;
        }
    } else {
        history11 = { id: 'history_11_cat', name: 'XI sinif', time: 45, questions: [], parentId: 'az_history_cat' };
        categories.push(history11);
        if (db) db.collection('categories').doc('history_11_cat').set(history11);
        hasChanged = true;
    }
...
window.savePrivateQuizFinal = async function() {
    ...
    const quizData = {
        ...
        title: title,
        questions: questions, // Save plain
        password: password,   // Save plain
        timeType: timeType,
...
window.renderPrivateQuizzes = async function() {
    ...
    myQuizzes.forEach(quiz => {
        ...
        card.innerHTML = `
            ...
            ${quiz.password ? `<p style="font-size: 0.9em; color: #666; margin-top: 5px;">Şifrə: <strong>${quiz.password}</strong></p>` : ''}
            ...
            if (attempts.length > 0 && quizData) {
                if (Array.isArray(quizData.questions)) {
                    ...
                    try {
                         ...
                         var savedPwd = localStorage.getItem('quiz_pass_' + quizId) || quizData.password;
                         if (savedPwd) {
                             var bytes = CryptoJS.AES.decrypt(quizData.questionsCipher, savedPwd);
...
function handleUrlParams() {
    ...
    // Public category/test deep link
    if (catId) {
        ...
        if (category) {
            ...
            const hasSub = categories.some(c => c.parentId === category.id);
            if (hasSub) {
                ...
                renderCategories();
                setTimeout(() => {
                    startQuizCheck(category.id);
                    const card = document.querySelector(`.category-card[data-id="${category.id}"]`);
                    if (card) {
...
window.accessPrivateQuiz = async function() {
    ...
    try {
        ...
        var valid=false;if(quiz.password){valid=pass===quiz.password}if(!valid){throw new Error('Yanlış şifrə!')}
