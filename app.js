...
window.migrateProkurorluqRootCategory = async function(opts) {
    ...
    categories = categories.map(c => String(c.id) === rootId ? { ...root, questions: qs } : c);

    const persist = (opts.persist !== false) && !!db;
    if (!persist) return { ok: true, persisted: false, mutatedQuestions, totalQuestions: qs.length };

    const version = 1;
    const already = (root.rootLegacyMigrationVersion != null) ? Number(root.rootLegacyMigrationVersion) : 0;
    if (already >= version && !needsModeUpdate && mutatedQuestions === 0) return { ok: true, persisted: false, mutatedQuestions, totalQuestions: qs.length };
    ...
    try {
        ...
        return { ok: false, reason: 'persist_root_failed', error: e && e.message ? e.message : String(e), mutatedQuestions, totalQuestions: qs.length };
    }

    const writeLegacyDocs = (opts.writeLegacyDocs !== false) && qs.length > 0;
    if (!writeLegacyDocs) return { ok: true, persisted: true, mutatedQuestions, totalQuestions: qs.length };
    ...
    for (let i = 0; i < take; i++) {
        ...
        const payload = { ...q, id: qid, categoryId: rootId, legacy: true, deleted: true, legacyReason: legacyReason, migratedAt: firebase.firestore.FieldValue.serverTimestamp() };
    ...
    return { ok: true, persisted: true, mutatedQuestions, totalQuestions: qs.length, legacyDocsWritten: written, legacyDocsLimited: take !== qs.length };
};
